...
import { ejecutarCierreMensual, listarCierres, eliminarCierre } from "../api/cierres";
...
function ModalGasto({ onClose, onGuardado, trabajadores }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      onGuardado();
    } catch (err) { setErr(parsearError(err, "Error al guardar.")); }
...
export default function FinanzasPage() {
  ...
  const [cargandoCierres, setCargC]     = useState(false);
  const [ejecutandoCierre, setEjecC]    = useState(false);
  const [eliminandoCierre, setElimC]    = useState(null);
  const [cierreMsg, setCierreMsg]       = useState({ tipo: "", texto: "" });
        ...
        <>
                          ...
                          <tr key={g.id_gasto} style={{ borderBottom: idx < datos.gastos_del_mes.length - 1 ? "1px solid #1f1f1f" : "none" }}>
                            ...
                            <td style={{ padding: "9px 12px" }}>
                              <button
                                ...
                                onClick={async () => { setEliminando(g.id_gasto); try { await eliminarGasto(g.id_gasto); cargar(anio, mes); } catch (err) { setError(parsearError(err, "Error al eliminar gasto.")); } finally { setEliminando(null); } }}
          ...
          <div style={{ marginTop: 12, marginBottom: 20 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
              <div>
                <p style={{ fontSize: 12, color: "#6b7280", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 2 }}>
                  🔒 Cierre de Caja
                </p>
                <p style={{ fontSize: 13, color: "#4b5563", margin: 0 }}>
                  Cierra el mes seleccionado y guardá un resumen histórico inmutable.
                </p>
              </div>
              <button
                onClick={async () => {
                  setEjecC(true); setCierreMsg({ tipo: "", texto: "" });
                  try {
                    await ejecutarCierreMensual(anio, mes);
                    setCierreMsg({ tipo: "ok", texto: `✅ Cierre de ${MESES_ES[mes-1]} ${anio} ejecutado con éxito.` });
                    cargarCierres();
                  } catch (e) {
                    const detail = e?.response?.data?.detail || "Error al ejecutar el cierre.";
                    setCierreMsg({ tipo: "err", texto: `⚠ ${detail}` });
                  } finally { setEjecC(false); }
                }}
                disabled={ejecutandoCierre}
                style={{
                  display: "flex", alignItems: "center", gap: 8,
                  padding: "10px 22px", borderRadius: 10,
                  backgroundColor: ejecutandoCierre ? "#374151" : "#6d28d9",
                  border: "none", color: "#fff", fontSize: 13, fontWeight: 700,
                  cursor: ejecutandoCierre ? "not-allowed" : "pointer",
                  transition: "background 0.2s",
                }}
                onMouseEnter={e => { if (!ejecutandoCierre) e.currentTarget.style.backgroundColor = "#5b21b6"; }}
                onMouseLeave={e => { if (!ejecutandoCierre) e.currentTarget.style.backgroundColor = "#6d28d9"; }}
              >
                🔒 {ejecutandoCierre ? "Cerrando..." : `Cerrar ${MESES_ES[mes-1]} ${anio}`}
              </button>
            </div>
            ...
            {cierreMsg.texto && (
              <div style={{
                padding: "10px 16px", borderRadius: 8, marginBottom: 14, fontSize: 13, fontWeight: 600,
                backgroundColor: cierreMsg.tipo === "ok" ? "#16a34a22" : "#ef444422",
                border: `1px solid ${cierreMsg.tipo === "ok" ? "#16a34a55" : "#ef444455"}`,
                color: cierreMsg.tipo === "ok" ? "#22c55e" : "#ef4444",
              }}>
                {cierreMsg.texto}
              </div>
            )}
            ...
            <div style={{ ...CARD, border: "1px solid #6d28d933", background: "linear-gradient(135deg, #1a1a1a 60%, #6d28d908)" }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
                <p style={{ color: "#c4b5fd", fontWeight: 700, fontSize: 15, margin: 0 }}>📋 Historial de cierres</p>
                <p style={{ color: "#6b7280", fontSize: 12, margin: 0 }}>{cierres.length} cierre{cierres.length !== 1 ? "s" : ""} registrado{cierres.length !== 1 ? "s" : ""}</p>
              </div>

              {cargandoCierres ? (
                <div style={{ display: "flex", justifyContent: "center", padding: 24 }}>
                  <div style={{ width: 24, height: 24, borderRadius: "50%", border: "3px solid #6d28d9", borderTopColor: "transparent", animation: "spin 0.7s linear infinite" }} />
                </div>
              ) : cierres.length === 0 ? (
                <div style={{ textAlign: "center", padding: "28px 0", color: "#4b5563" }}>
                  <div style={{ fontSize: 32, marginBottom: 8 }}>📂</div>
                  <p style={{ margin: 0, fontSize: 13 }}>Aún no hay cierres registrados.</p>
                  <p style={{ margin: "4px 0 0", fontSize: 12 }}>Ejecutá el cierre del mes para guardar un resumen histórico.</p>
                </div>
              ) : (
                <div style={{ overflowX: "auto" }}>
                  <table style={{ width: "100%", borderCollapse: "collapse" }}>
                    <thead>
                      <tr style={{ borderBottom: "1px solid #2a2a2a" }}>
                        {["Período", "Tipo", "Ingresos", "Egresos", "Balance", "Fecha cierre", ""].map(h => (
                          <th key={h} style={{ textAlign: "left", padding: "8px 12px", fontSize: 11, fontWeight: 700, textTransform: "uppercase", color: "#6b7280" }}>{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {cierres.map((c, idx) => {
                        const bal = (c.total_ingresos || 0) - (c.total_egresos || 0);
                        const pos = bal >= 0;
                        return (
                          <tr key={c.id_cierre} style={{ borderBottom: idx < cierres.length - 1 ? "1px solid #1f1f1f" : "none" }}>
                            <td style={{ padding: "10px 12px", color: "#fff", fontWeight: 700, fontSize: 14 }}>{c.periodo}</td>
                            <td style={{ padding: "10px 12px" }}>
                              <span style={{ fontSize: 11, padding: "2px 8px", borderRadius: 999, backgroundColor: "#6d28d922", color: "#c4b5fd", fontWeight: 700 }}>
                                {c.tipo || "mensual"}
                              </span>
                            </td>
                            <td style={{ padding: "10px 12px", color: "#22c55e", fontWeight: 700 }}>
                              +${Number(c.total_ingresos || 0).toLocaleString("es-AR")}
                            </td>
                            <td style={{ padding: "10px 12px", color: "#ef4444", fontWeight: 700 }}>
                              -${Number(c.total_egresos || 0).toLocaleString("es-AR")}
                            </td>
                            <td style={{ padding: "10px 12px" }}>
                              <span style={{ fontWeight: 800, fontSize: 14, color: pos ? "#22c55e" : "#ef4444" }}>
                                {pos ? "+" : ""}${Number(bal).toLocaleString("es-AR")}
                              </span>
                            </td>
                            <td style={{ padding: "10px 12px", color: "#6b7280", fontSize: 12 }}>
                              {c.fecha_cierre ? new Date(c.fecha_cierre).toLocaleDateString("es-AR") : "—"}
                            </td>
                            <td style={{ padding: "10px 12px" }}>
                              <button
                                disabled={eliminandoCierre === c.id_cierre}
                                onClick={async () => {
                                  setElimC(c.id_cierre);
                                  try { await eliminarCierre(c.id_cierre); cargarCierres(); }
                                  catch (err) { setCierreMsg({ tipo: "err", texto: parsearError(err, "Error al eliminar cierre.") }); } finally { setElimC(null); }
                                }}
                                style={{ padding: "4px 10px", borderRadius: 6, border: "1px solid #ef444444", backgroundColor: "#ef444415", color: "#ef4444", fontSize: 11, fontWeight: 700, cursor: "pointer" }}
                              >
                                {eliminandoCierre === c.id_cierre ? "..." : "Eliminar"}
                              </button>
                            </td>
                          </tr>
                        );
